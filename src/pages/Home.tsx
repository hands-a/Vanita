...
// Updated images for products to look better with object-contain
const topPicksData = [
...
const Home: React.FC = () => {
  return (
    <div className="w-full font-outfit text-gray-800">
      ...
      <section className="relative w-full h-[85vh] md:h-[90vh] overflow-hidden bg-[#f4f4f4]">
        <Swiper
          ...
          {sliderData.map((slide) => (
            <SwiperSlide key={slide.id} className="w-full h-full">
              <div className="flex flex-col md:flex-row w-full h-full">
                ...
                {/* Left Side: Text */}
                <div 
                  className="flex-1 flex flex-col justify-center items-center text-white px-8 py-16 md:py-12 text-center h-full relative" 
                  style={{ backgroundColor: slide.bgColor }}
                >
                  <motion.h1 
                    initial={{ opacity: 0, y: 30 }} key={`title_${slide.id}`} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: "easeOut" }}
                    className="font-playfair text-5xl md:text-6xl lg:text-7xl font-bold mb-6 leading-tight tracking-wide"
                  >
                    {slide.title}
                  </motion.h1>
                  ...
                  <motion.p 
                    ...
                    className="text-lg md:text-xl mb-10 max-w-md text-white/90 font-light"
                  >
                    {slide.subtitle}
                  </motion.p>
                  
                  <motion.div initial={{ opacity: 0, scale: 0.9 }} key={`btn_${slide.id}`} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.4 }}>
                    <Link to="/shop" className="inline-block bg-white text-gray-900 px-10 py-4 rounded-full text-lg font-medium hover:bg-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                      Shop Collection
                    </Link>
                  </motion.div>
                </div>
                ...
                {/* Right Side: Image */}
                <div className="flex-1 relative overflow-hidden bg-gray-200 h-full min-h-[400px] md:min-h-full">
                  <motion.img 
                  ...
                  />
                  {/* Overlay for a more premium look */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {servicesData.map((service, index) => (
              <motion.div
                ...
                className="bg-white border border-gray-100 rounded-3xl p-10 text-center hover:shadow-[0_10px_40px_rgba(0,0,0,0.06)] transition-all duration-500 cursor-pointer group"
              >
                <div className="w-20 h-20 bg-[#f9f5f3] rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 group-hover:bg-[#8f4a58] transition-all duration-500">
                  <div className="group-hover:text-white transition-colors duration-500">
                    {service.icon}
                  </div>
                </div>
                <h2 className="font-playfair font-bold text-2xl pt-2 pb-3 text-gray-900">{service.title}</h2>
        ...
        <div className="container mx-auto">
          <div className="text-center mb-16">
            <motion.h1 
              ...
              className="font-playfair font-bold text-4xl md:text-5xl mb-6 text-gray-900"
            >
              Top Picks You'll Love
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="text-gray-500 max-w-2xl mx-auto font-light text-lg">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {topPicksData.map((product, index) => (
              <motion.div 
                ...
                className="bg-white rounded-3xl overflow-hidden hover:shadow-[0_15px_50px_rgba(0,0,0,0.08)] transition-all duration-500 group border border-gray-100"
              >
                
                <div className="relative h-[340px] bg-[#f4f5f7] flex items-center justify-center p-6 overflow-hidden">
                  {product.badge && (
                    <span className={`absolute top-5 left-5 z-20 text-white text-[11px] uppercase tracking-wider font-bold px-4 py-1.5 rounded-full ${product.badgeColor}`}>
                      {product.badge}
                    </span>
                  )}
                  ...
                  <div className="absolute top-5 right-5 flex flex-col gap-3 opacity-0 translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-400 z-20">
                    <button className="w-11 h-11 bg-white text-gray-700 rounded-full flex items-center justify-center shadow-lg hover:bg-[#8f4a58] hover:text-white transition-colors duration-300" title="Add to Cart"><FaShoppingBag /></button>
                    <button className="w-11 h-11 bg-white text-gray-700 rounded-full flex items-center justify-center shadow-lg hover:bg-[#ff6f61] hover:text-white transition-colors duration-300" title="Add to Wishlist"><FaHeart /></button>
                    <button className="w-11 h-11 bg-white text-gray-700 rounded-full flex items-center justify-center shadow-lg hover:bg-[#ba9272] hover:text-white transition-colors duration-300" title="Quick View"><FaEye /></button>
                  </div>
                </div>

                <div className="p-6">
                  <Link to={`/product/${product.id}`} className="font-playfair font-bold text-xl text-gray-900 hover:text-[#8f4a58] transition-colors duration-300 block truncate">
                    {product.name}
                  </Link>
                  <div className="flex items-center pt-3">
                    <span className="text-[#8f4a58] font-bold text-xl">${product.price.toFixed(2)}</span>
                    {product.oldPrice && <span className="text-gray-400 font-medium text-sm line-through ml-3">${product.oldPrice.toFixed(2)}</span>}
                  </div>
                  <div className="flex gap-2 pt-5">
                    {product.colors.map((color, i) => (
                      <span key={i} className="w-6 h-6 rounded-full border-2 border-white shadow-sm cursor-pointer hover:scale-110 transition-transform" style={{ backgroundColor: color }} title="Select color"></span>
                    ))}
                  </div>
                </div>
              </motion.div>
        ...
        <div className="container mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <motion.div 
              ...
              className="lg:col-span-2 rounded-3xl overflow-hidden relative group h-[450px] cursor-pointer shadow-sm"
            >
              ...
              <div className="absolute inset-0 bg-black/20 flex items-center justify-center group-hover:bg-black/40 transition-colors duration-500">
                <Link to="/shop?category=chairs" className="font-playfair bg-white text-gray-900 px-12 py-4 rounded-full text-2xl font-bold hover:bg-[#8f4a58] hover:text-white transition-colors duration-400 transform hover:scale-105 shadow-xl">
                  Chairs Collection
                </Link>
              </div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 gap-8">
              <motion.div 
                ...
                className="rounded-3xl overflow-hidden relative group h-full min-h-[210px] cursor-pointer shadow-sm"
              >
                ...
                <div className="absolute inset-0 bg-black/20 flex items-center justify-center group-hover:bg-black/40 transition-colors duration-500">
                  <Link to="/shop?category=tables" className="font-playfair bg-white text-gray-900 px-10 py-3 rounded-full text-xl font-bold hover:bg-[#ba9272] hover:text-white transition-colors duration-400 transform hover:scale-105 shadow-xl">
                    Tables
                  </Link>
                </div>
              </motion.div>
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/20 flex items-center justify-center group-hover:bg-black/40 transition-colors duration-500">
                  <Link to="/shop?category=decor" className="font-playfair bg-white text-gray-900 px-10 py-3 rounded-full text-xl font-bold hover:bg-[#ff6f61] hover:text-white transition-colors duration-400 transform hover:scale-105 shadow-xl">
                    Decorations
                  </Link>
                </div>
              </motion.div>
      ...
      <section className="py-20 px-4 bg-[#fcfcfc]">
        <div className="container mx-auto">
          <div className="text-center mb-16">
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="text-gray-500 font-light text-lg">
            ...
            {curatedData.map((item, index) => (
              ...
              >
                <Link to={`/shop?category=${item.title.toLowerCase()}`}>
                  <div className="rounded-full overflow-hidden mb-6 border-[6px] border-transparent group-hover:border-[#ba9272]/20 transition-all duration-500 shadow-sm group-hover:shadow-xl">
                    <img 
                      src={item.image} 
                      alt={item.title} 
                      className="w-full h-36 md:h-44 object-cover transform group-hover:scale-110 transition-transform duration-700" 
                    />
                  </div>
                  <p className="font-playfair font-bold text-xl text-gray-800 group-hover:text-[#8f4a58] transition-colors">{item.title}</p>
                </Link>
              </motion.div>
            ))}
          </div>
